...
const getPropertyNotificationTitle = (eventType: string): string => {
  switch (eventType) {
    case 'INSERT': return 'Nueva Propiedad'
    case 'UPDATE': return 'Propiedad Actualizada'
    case 'DELETE': return 'Propiedad Eliminada'
...
const getPropertyNotificationMessage = (eventType: string, title?: string): string => {
  ...
  switch (eventType) {
    case 'INSERT': return `${propertyName} ha sido agregada al sistema`
    case 'UPDATE': return `${propertyName} ha sido actualizada`
    case 'DELETE': return `${propertyName} ha sido eliminada`
    default: return `${propertyName} ha sido modificada`
  }
}

const getLeadNotificationTitle = (eventType: string): string => {
  switch (eventType) {
    case 'INSERT': return 'Nuevo Lead'
    case 'UPDATE': return 'Lead Actualizado'
    case 'DELETE': return 'Lead Eliminado'
    default: return 'Lead'
  }
}
...
const getLeadNotificationMessage = (eventType: string, name?: string): string => {
  ...
  switch (eventType) {
    case 'INSERT': return `Nuevo lead de ${leadName}`
    case 'UPDATE': return `Lead de ${leadName} ha sido actualizado`
    case 'DELETE': return `Lead de ${leadName} ha sido eliminado`
    default: return `Lead de ${leadName} ha sido modificado`
  }
}